    ...
    #print(classIds,bbox)
            ...
            for c in cnts:
                ...
                if label=='COW' or label=='DOG' or label=='BIRD' or label=='CAT' or label=='HORSE':
                    imageio.imwrite('./animal_imgs/outfile'+str(imgCounter)+'.jpg', frame )
                    # Get the current directory path
                    current_directory = os.path.dirname(os.path.abspath(__file__))

                    # Configure Cloudinary with your account credentials
                    cloudinary.config(
                    cloud_name = "do09fscgf",
                    api_key = "279544317861435",
                    api_secret = "ZDof-N206dpfpPvwC4nwFhky-4Q"
                    #   secure = true
                    )

                    # Construct the full path to the image file
                    image_path = os.path.join(current_directory, 'animal_imgs/outfile'+str(imgCounter)+'.jpg')
            ...
            # otherwise, the room is not occupied
                ...
                print(val)
                # val=20 shows that image will be generated if the vehicle stands for more than 20secs
                if int(val)>= 20 and (label=='CAR' or label =='TRUCK' or label=='MOTORCYCLE' or label=='BICYCLE' or label=='BUS'):
                    ...
                    # Construct the full path to the image file
                    image_path = os.path.join(current_directory, 'saved_imgs/outfile'+str(imgCounter)+'.jpg')
